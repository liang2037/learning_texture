...
void My_Mesh::generate_disk(int num_division)
{
	...
	float g;
	float b;
	My_Mesh::normal_to_color(0, 0, z, r, g, b);
	//这里采用法线来生成颜色，学生可以自定义自己的颜色生成方式
	m_color_list_.push_back(r);
	m_color_list_.push_back(g);
	m_color_list_.push_back(b);
	...
	for (int i = 0; i < num_samples; i++)
	{
		...
		m_vt_list_.push_back(0.5 + 0.5 * cos(1.0 * i / num_samples));
		m_vt_list_.push_back(0.5 + 0.5 * sin(1.0 * i / num_samples));
		//纹理坐标
		m_vt_list_.push_back(0.5 + 0.5 * cos(1.0 * (i + 1) / num_samples));
		m_vt_list_.push_back(0.5 + 0.5 * sin(1.0 * (i + 1) / num_samples));
		//纹理坐标
